...
static int
packet_extract_addresses(uint8_t *buf, uint8_t len, linkaddr_t *source_address, linkaddr_t *dest_address)
{
  frame802154_t frame;
  ...
  if(parsed) {
    ...
    if(frame.fcf.dest_addr_mode) {
      if(frame.dest_pid != IEEE802154_PANID
        ...
        PRINTF("packet_extract_addresses: for another pan %u\n", frame.dest_pid);
        return 0;
      }
      ...
      if(frame.fcf.src_addr_mode && frame.src_pid != IEEE802154_PANID) {
        ...
        PRINTF("packet_extract_addresses: from another pan %u\n", frame.src_pid);
        return 0;
      }
    }
    ...
    PRINTF("packet_extract_addresses: failed to parse\n");
  }
  return parsed;
}
static int
micromac_radio_is_packet_for_us(uint8_t* data, int len)
{
  ...
  uint8_t parsed = packet_extract_addresses(data, len, NULL, &dest_address);

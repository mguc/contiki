...
docker run --rm -a stdin -a stdout -a stderr -v $(pwd)/dist:/out -v $(pwd):/source tr2contiki bash -c 'cd /source/neeo/tr2-com-firmware && PATH=$PATH:/builder/bin WITH_FEM=1 TR2=1 TARGET=jn516x-neeo make distclean all && cp ./tr2-com-firmware.jn516x-neeo.bin /out/jn516x-tr2.bin && CONTIKI_VERSION=$(grep -h "#define FW_MAJOR_VERSION " tr2-com-firmware.c | tr -s " " | cut -d\  -f 3- | tr -d \") && printf -v v "%08x" $CONTIKI_VERSION && echo ${v:6:2}${v:4:2}${v:2:2}${v:0:2} | while read -N2 code; do printf "\x$code"; done | dd conv=notrunc of=/out/jn516x-tr2.bin bs=1 count=4'

# build cp6 JN5168 binary
docker run --rm -a stdin -a stdout -a stderr -v $(pwd)/dist:/out -v $(pwd):/source tr2contiki bash -c 'cd /source/neeo/slip-radio && PATH=$PATH:/builder/bin WITH_FEM=1 TARGET=jn516x-neeo make distclean all && cp ./slip-radio.jn516x-neeo.bin /out/jn516x-cp6.bin'

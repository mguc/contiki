...
public:
    ...
    void Clear() {
        ...
        while( !_blockPtrs.Empty()) {
            ...
            if (b !=  NULL) {
                if(guilib::GUILib::Callbacks()->free) {
                    guilib::GUILib::Callbacks()->free(b);
                } else {
                     delete b;
                }
            }
    ...
    virtual void* Alloc() {
        if ( !_root ) {
            // Need a new block.
            Block* block = 0;
            if(guilib::GUILib::Callbacks()->malloc){
                void* externalRam = guilib::GUILib::Callbacks()->malloc(sizeof(Block)+1);
                block = new(externalRam) Block();
            } else {
                block = new Block();
            }

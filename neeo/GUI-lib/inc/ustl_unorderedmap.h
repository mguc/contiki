/***********************************************************************************************
*    This is a basic unordered_map implementation for ustl library created @ Antmicro Ltd.
*
*    Author: Łukasz Kucharski <lkucharski@antmicro.com>
************************************************************************************************/
...
namespace ustl {
  ...
  public:
    ...
    inline float		load_factor () const 	{ return (float)_size/bucket_count(); };
  protected:
  ...
  unordered_map<Key, T, Hash, KeyEqual>::unordered_map() : _buckets(default_buckets), _hash(), _key_comparer(), _size(0)
  {
  }

  template<class Key, class T, class Hash, class KeyEqual>
  unordered_map<Key, T, Hash, KeyEqual>::unordered_map(size_type bucket_count) : _buckets(bucket_count), _hash(), _key_comparer(), _size(0)
  {
  }
  ...
  {
    //TODO: make c++11 compilant version with emplace if eCos supports it
  ...
  typename unordered_map<Key, T, Hash, KeyEqual>::insertrv_t unordered_map<Key, T, Hash, KeyEqual>::insert(const_reference v)
  {
    ...
    if(inserted) {
      ...
      _size++;
//      cout << "Inserted element into bucket: " << bucket_idx << ", current bucket size: " << _buckets[bucket_idx].size() << ", size:" << _size << ", byte_size: " << byte_size() << " load factor: " << load_factor()<< "\r\n";
    }

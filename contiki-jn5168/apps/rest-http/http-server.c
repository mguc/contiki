...
PT_THREAD(send_data(connection_state_t* conn_state))
{
  ...
  if (buffer) {
    index += sprintf(buffer + index, "%s %d %s%s", http_httpv1_1, response->status_code, response->status_string, http_line_end);
    for (;header;header = header->next) {
      PRINTF("header %u \n", (uint16_t)header);
      index += sprintf(buffer + index, "%s%s%s%s", header->name, header_delimiter, header->value, http_line_end);
    }
    index += sprintf(buffer + index, "%s", http_line_end);
...
PT_THREAD(handle_response(connection_state_t* conn_state))
{
  ...
  http_set_res_header(&conn_state->response, HTTP_HEADER_NAME_CONNECTION, http_close, 0);
  http_set_res_header(&conn_state->response, HTTP_HEADER_NAME_SERVER, http_contiki, 0);
